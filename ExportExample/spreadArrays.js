...
//And no matter how many times we call it, the result will be the same
numbers.push(10);
const modifiedNumbers = numbers.concat(12);

//As a rule of thumb, prefer pure functions over impure functions
//In terms of arrays, use functions that return new arrays
...
//and we can also modify existing properties by not modifying original object
const willyReborn = { ...willy, reborn: true };
const willyRebornOlder = { ...willyReborn, age: 500 };
//This approach is not pure, because it modifies the original object
willyReborn.age = 500;
...
const doubledDozen = dozen.map((number) => Math.pow(number, 2));
...
//4) accumulator = 6, number = undefined
